...
BEGIN
    ...
    DECLARE plan_is_cluster TINYINT;
    DECLARE plan_cluster_no INT;
    DECLARE plan_cluster_pos INT;
    ...
    DECLARE special_id INT DEFAULT 1;
    DECLARE special_max_id INT DEFAULT 0;
    DECLARE special_date DATE;
    DECLARE special_shift VARCHAR(20);
    DECLARE special_rule_id INT;
    DECLARE special_minute_offset INT;
    DECLARE special_action_status VARCHAR(20);
    DECLARE special_session_id INT;

    DECLARE cur_event_plan CURSOR FOR
        SELECT rule_id, seq_no, is_cluster, cluster_no, cluster_pos
        FROM temp_event_plan
        ORDER BY sort_key, is_cluster DESC, cluster_pos;

    DECLARE CONTINUE HANDLER FOR NOT FOUND SET done = TRUE;

    SET current_day = '2025-05-11';
    SET end_day = '2026-05-27';
    ...
    -- 2) AI 보고서 기록실용 특별 구간 계획표
    -- 2026-05-21~2026-05-27에 최근 보고서용 이벤트를 의도적으로 배치
    -- 마지막 보고서(2026-05-27 야간)는 5가지 케이스가 모두 포함되도록 구성
    DROP TEMPORARY TABLE IF EXISTS temp_special_plan;
    CREATE TEMPORARY TABLE temp_special_plan (
    ...
    INSERT INTO temp_special_plan (plan_date, shift_type, rule_id, minute_offset, action_status) VALUES
    ('2026-05-21', '오전', 1,  75, '조치완료'),
    ('2026-05-21', '오전', 4, 210, '조치완료'),
    ('2026-05-21', '오후', 2,  95, '조치완료'),
    ('2026-05-21', '오후', 3, 260, '미조치'),
    ('2026-05-21', '야간', 5, 120, '조치완료'),
    ('2026-05-22', '오전', 2,  60, '조치완료'),
    ('2026-05-22', '오전', 5, 190, '조치완료'),
    ('2026-05-22', '오후', 1, 130, '미조치'),
    ('2026-05-22', '오후', 4, 310, '조치완료'),
    ('2026-05-22', '야간', 3, 180, '조치완료'),
    ('2026-05-25', '오전', 4,  85, '조치완료'),
    ('2026-05-25', '오전', 3, 240, '조치완료'),
    ('2026-05-25', '오후', 5, 100, '조치완료'),
    ('2026-05-25', '오후', 1, 280, '미조치'),
    ('2026-05-25', '야간', 2, 155, '조치완료'),
    ('2026-05-26', '오전', 1,  70, '조치완료'),
    ('2026-05-26', '오전', 2, 225, '조치완료'),
    ('2026-05-26', '오후', 3, 115, '조치완료'),
    ('2026-05-26', '오후', 4, 275, '조치완료'),
    ('2026-05-26', '야간', 5, 160, '미조치'),
    ('2026-05-26', '야간', 2, 330, '조치완료'),
    ('2026-05-27', '오전', 4,  90, '조치완료'),
    ('2026-05-27', '오전', 1, 260, '조치완료'),
    ('2026-05-27', '오후', 2, 105, '조치완료'),
    ('2026-05-27', '오후', 5, 245, '조치완료'),
    ('2026-05-27', '야간', 1,  45, '조치완료'),
    ('2026-05-27', '야간', 2, 110, '조치완료'),
    ('2026-05-27', '야간', 3, 185, '미조치'),
    ('2026-05-27', '야간', 4, 260, '조치완료'),
    ('2026-05-27', '야간', 5, 350, '조치완료');

    -- 3) 사고유형별 목표 건수는 2024년 건설업 통계의 1/10로 유지
    -- 단, 최근 AI 보고서용 특별 이벤트 수만큼 일반 생성량에서 차감
    -- 일반 생성량 안에서는 약 20%를 같은 세션 안에서 반복 감지되는 클러스터 이벤트로 배치
    ...
    CREATE TEMPORARY TABLE temp_event_plan (
        ...
        rule_id INT NOT NULL,
        seq_no INT NOT NULL,
        is_cluster TINYINT NOT NULL DEFAULT 0,
        cluster_no INT NULL,
        cluster_pos INT NULL,
        sort_key INT NOT NULL
    );

    INSERT INTO temp_event_plan (rule_id, seq_no, is_cluster, cluster_no, cluster_pos, sort_key)
    SELECT
        sr.rule_id,
        s.n,
        CASE WHEN s.n <= FLOOR((FLOOR(sr.accident_count_2024 / 10) - COALESCE(sp.special_count, 0)) * 0.20) THEN 1 ELSE 0 END,
        CASE WHEN s.n <= FLOOR((FLOOR(sr.accident_count_2024 / 10) - COALESCE(sp.special_count, 0)) * 0.20) THEN CEIL(s.n / 3) ELSE NULL END,
        CASE WHEN s.n <= FLOOR((FLOOR(sr.accident_count_2024 / 10) - COALESCE(sp.special_count, 0)) * 0.20) THEN MOD(s.n - 1, 3) + 1 ELSE NULL END,
        CASE
            WHEN s.n <= FLOOR((FLOOR(sr.accident_count_2024 / 10) - COALESCE(sp.special_count, 0)) * 0.20)
                THEN MOD(CEIL(s.n / 3) * 587 + sr.rule_id * 389, 1000003)
            ELSE MOD(s.n * 97 + sr.rule_id * 389, 1000003)
        END
    FROM safety_rules sr
    JOIN temp_seq s
        ON s.n <= (FLOOR(sr.accident_count_2024 / 10) - COALESCE((
            SELECT COUNT(*)
            FROM temp_special_plan tsp
            WHERE tsp.rule_id = sr.rule_id
        ), 0))
    LEFT JOIN (
        SELECT rule_id, COUNT(*) AS special_count
        FROM temp_special_plan
        GROUP BY rule_id
    ) sp ON sr.rule_id = sp.rule_id;

    -- 4) 섞인 순서대로 위험 이벤트 생성
    -- 일반 이벤트는 전체 기간에 분산하고, 클러스터 이벤트는 같은 세션 안에 30~40분 간격으로 반복 배치
    OPEN cur_event_plan;

    event_loop: LOOP
        FETCH cur_event_plan INTO plan_rule_id, plan_seq_no, plan_is_cluster, plan_cluster_no, plan_cluster_pos;
        ...
        IF plan_is_cluster = 1 THEN
            SET target_session_id = MOD(plan_rule_id * 251 + plan_cluster_no * 29, total_sessions) + 1;
        ELSE
        ...
        IF plan_is_cluster = 1 THEN
            SET detected_dt = DATE_ADD(
                target_start_time,
                INTERVAL LEAST(450, 60 + ((plan_cluster_pos - 1) * 40) + MOD(plan_cluster_no * 7, 25)) MINUTE
            );
        ELSE
            SET detected_dt = DATE_ADD(
                target_start_time,
        ...
        (target_session_id, plan_rule_id, detected_dt, selected_case_name, selected_accident_type,
         ...
         CONCAT(selected_case_name, ' 상황이 감지되었습니다. 대표 사고유형은 ', selected_accident_type,
                ...
                CASE WHEN plan_is_cluster = 1 THEN ' 동일 유형이 같은 근무 세션 안에서 반복 감지된 사례입니다.' ELSE '' END),
    ...
    -- 5) AI 보고서 기록실용 특별 이벤트 삽입
    SELECT MAX(id) INTO special_max_id FROM temp_special_plan;

    WHILE special_id <= special_max_id DO
        SELECT plan_date, shift_type, rule_id, minute_offset, action_status
        INTO special_date, special_shift, special_rule_id, special_minute_offset, special_action_status
        FROM temp_special_plan
        WHERE id = special_id;

        SELECT session_id, start_time, manager_id
        INTO special_session_id, target_start_time, target_manager_id
        FROM monitoring_sessions
        WHERE session_date = special_date
          AND shift_type = special_shift
        LIMIT 1;

        SELECT case_name, accident_type, likelihood_score, severity_score, risk_score, risk_percent, risk_level
        INTO selected_case_name, selected_accident_type, selected_likelihood_score, selected_severity_score, selected_risk_score, selected_risk_percent, selected_risk_level
        FROM safety_rules
        WHERE rule_id = special_rule_id;

        SET detected_dt = DATE_ADD(target_start_time, INTERVAL special_minute_offset MINUTE);
        SET global_risk_counter = global_risk_counter + 1;
        ...
        (session_id, rule_id, detected_time, risk_case, accident_type,
         ...
         description, image_path, bbox_image_path, created_at)
        VALUES
        (special_session_id, special_rule_id, detected_dt, selected_case_name, selected_accident_type,
         selected_likelihood_score, selected_severity_score, selected_risk_score, selected_risk_percent, selected_risk_level,
         CONCAT('[AI 보고서용 최근 기록] ', selected_case_name, ' 상황이 감지되었습니다. 위험도는 ',
                selected_risk_percent, '%입니다.'),
         CONCAT('/uploads/dummy/ai_reports/original/event_', LPAD(global_risk_counter, 5, '0'), '.jpg'),
         CONCAT('/uploads/dummy/ai_reports/bbox/event_', LPAD(global_risk_counter, 5, '0'), '_bbox.jpg'),
         detected_dt);

        SET new_risk_id = LAST_INSERT_ID();

        IF special_rule_id = 1 THEN
            INSERT INTO detection_results (risk_id, object_name, detected_count) VALUES
            (new_risk_id, 'person', 1),
            (new_risk_id, 'danger_zone', 1);
        ELSEIF special_rule_id = 2 THEN
            INSERT INTO detection_results (risk_id, object_name, detected_count) VALUES
            (new_risk_id, 'box', 3),
            (new_risk_id, 'pallet', 1);
        ELSEIF special_rule_id = 3 THEN
            INSERT INTO detection_results (risk_id, object_name, detected_count) VALUES
            (new_risk_id, 'person', 1),
            (new_risk_id, 'helmet', 0),
            (new_risk_id, 'vest', 0),
            (new_risk_id, 'boots', 0);
        ELSEIF special_rule_id = 4 THEN
            INSERT INTO detection_results (risk_id, object_name, detected_count) VALUES
            (new_risk_id, 'person', 6);
        ELSE
            INSERT INTO detection_results (risk_id, object_name, detected_count) VALUES
        ...
        (new_risk_id,
         ...
         target_manager_id,
         CASE WHEN special_action_status = '미조치' THEN NULL ELSE DATE_ADD(detected_dt, INTERVAL 15 MINUTE) END,
         detected_dt);

        SET special_id = special_id + 1;
    END WHILE;
...
-- AI 보고서용 최근 구간 확인: 2026-05-21 ~ 2026-05-27
SELECT
    ms.session_date,
    ms.shift_type,
    r.report_id,
...
JOIN monitoring_sessions ms ON r.session_id = ms.session_id
JOIN handover_logs hl ON r.report_id = hl.report_id
WHERE ms.session_date BETWEEN '2026-05-21' AND '2026-05-27'
ORDER BY ms.session_date, FIELD(ms.shift_type, '오전', '오후', '야간');

-- 마지막 보고서(2026-05-27 야간)에 5가지 사고 케이스가 모두 들어갔는지 확인
SELECT
    re.risk_case AS 마지막_보고서_사고케이스,
    re.accident_type,
    re.risk_percent,
    al.action_status
FROM risk_events re
JOIN monitoring_sessions ms ON re.session_id = ms.session_id
JOIN action_logs al ON re.risk_id = al.risk_id
WHERE ms.session_date = '2026-05-27'
  AND ms.shift_type = '야간'
ORDER BY re.detected_time;


-- 반복 감지 클러스터 확인: 같은 세션/같은 사고 케이스가 여러 번 감지된 사례
SELECT
    ms.session_date,
    ms.shift_type,
    re.risk_case,
    COUNT(*) AS repeated_count,
    MIN(re.detected_time) AS first_detected,
    MAX(re.detected_time) AS last_detected
FROM risk_events re
JOIN monitoring_sessions ms ON re.session_id = ms.session_id
GROUP BY ms.session_id, re.risk_case
HAVING COUNT(*) >= 2
ORDER BY repeated_count DESC, first_detected DESC
LIMIT 20;

-- 정상 세션과 위험 발생 세션 비율 확인
SELECT
    session_status,
    COUNT(*) AS session_count,
    ROUND(COUNT(*) / (SELECT COUNT(*) FROM monitoring_sessions) * 100, 1) AS percentage
FROM monitoring_sessions
