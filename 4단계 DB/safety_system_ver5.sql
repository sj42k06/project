...
-- 3단계: AI 객체 탐지 기능용 추가 테이블
-- 기존 2단계 테이블은 수정하지 않음
-- 12. AI 탐지 실행 기록 테이블
CREATE TABLE analysis_runs (
    analysis_run_id INT AUTO_INCREMENT PRIMARY KEY,
    batch_id INT NOT NULL COMMENT '어느 업로드 배치 기준의 분석인지',
    file_id INT NOT NULL COMMENT '어느 원본 파일을 분석했는지',
    scenario_id INT NOT NULL COMMENT '어떤 시나리오 기준으로 분석했는지',
    run_type ENUM('object_detection') NOT NULL DEFAULT 'object_detection' COMMENT '실행 유형',
    model_name VARCHAR(100) NOT NULL COMMENT '사용 모델명 (예: YOLOv8)',
    model_version VARCHAR(50) NULL COMMENT '모델 버전',
    status ENUM('pending','processing','completed','failed') NOT NULL DEFAULT 'pending' COMMENT '분석 상태',
    result_image_path VARCHAR(500) NULL COMMENT '박스가 그려진 결과 이미지 경로',
    result_json_path VARCHAR(500) NULL COMMENT '탐지 결과 JSON 경로',
    total_detected_count INT NOT NULL DEFAULT 0 COMMENT '탐지된 총 객체 수',
    started_at DATETIME NULL COMMENT '분석 시작 시각',
    finished_at DATETIME NULL COMMENT '분석 종료 시각',
    created_at DATETIME DEFAULT CURRENT_TIMESTAMP,

    CONSTRAINT fk_analysis_runs_batch
        FOREIGN KEY (batch_id) REFERENCES upload_batches(batch_id)
        ON DELETE CASCADE ON UPDATE CASCADE,

    CONSTRAINT fk_analysis_runs_file
        FOREIGN KEY (file_id) REFERENCES upload_files(file_id)
        ON DELETE CASCADE ON UPDATE CASCADE,

    CONSTRAINT fk_analysis_runs_scenario
        FOREIGN KEY (scenario_id) REFERENCES scenarios(scenario_id)
        ON DELETE CASCADE ON UPDATE CASCADE
) COMMENT='AI 객체 탐지 실행 기록';

-- 13. 객체 탐지 결과 테이블
CREATE TABLE detection_results (
    detection_result_id INT AUTO_INCREMENT PRIMARY KEY,
    analysis_run_id INT NOT NULL COMMENT '어느 분석 실행 결과인지',
    file_id INT NOT NULL COMMENT '어느 원본 파일에서 나온 탐지 결과인지',
    frame_id INT NULL COMMENT '영상 프레임 결과일 경우 extracted_frames.frame_id',
    class_name VARCHAR(100) NOT NULL COMMENT '탐지 객체명 (예: person, helmet)',
    confidence DECIMAL(5,4) NOT NULL COMMENT '탐지 신뢰도',
    x_min INT NOT NULL COMMENT '박스 왼쪽 상단 X',
    y_min INT NOT NULL COMMENT '박스 왼쪽 상단 Y',
    x_max INT NOT NULL COMMENT '박스 오른쪽 하단 X',
    y_max INT NOT NULL COMMENT '박스 오른쪽 하단 Y',
    created_at DATETIME DEFAULT CURRENT_TIMESTAMP,

    CONSTRAINT fk_detection_results_run
        FOREIGN KEY (analysis_run_id) REFERENCES analysis_runs(analysis_run_id)
        ON DELETE CASCADE ON UPDATE CASCADE,

    CONSTRAINT fk_detection_results_file
        FOREIGN KEY (file_id) REFERENCES upload_files(file_id)
        ON DELETE CASCADE ON UPDATE CASCADE,

    CONSTRAINT fk_detection_results_frame
        FOREIGN KEY (frame_id) REFERENCES extracted_frames(frame_id)
        ON DELETE CASCADE ON UPDATE CASCADE
) COMMENT='AI 객체 탐지 결과';

-- 인덱스
CREATE INDEX idx_analysis_runs_batch_id ON analysis_runs(batch_id);
CREATE INDEX idx_analysis_runs_file_id ON analysis_runs(file_id);
CREATE INDEX idx_analysis_runs_scenario_id ON analysis_runs(scenario_id);
CREATE INDEX idx_analysis_runs_status ON analysis_runs(status);

CREATE INDEX idx_detection_results_run_id ON detection_results(analysis_run_id);
CREATE INDEX idx_detection_results_file_id ON detection_results(file_id);
CREATE INDEX idx_detection_results_frame_id ON detection_results(frame_id);
CREATE INDEX idx_detection_results_class_name ON detection_results(class_name);

-- 4단계: 위험 구역 진입 감지 기능용 추가 테이블
-- 14. 위험 구역(ROI) 정의 테이블
CREATE TABLE danger_zones (
    danger_zone_id INT AUTO_INCREMENT PRIMARY KEY,
    scenario_id INT NOT NULL COMMENT '어떤 시나리오용 위험 구역인지',
    zone_id INT NULL COMMENT '실제 작업 구역(zones)과 연결할 경우',
    file_id INT NULL COMMENT '특정 이미지/영상 파일 기준 ROI일 경우',
    zone_name VARCHAR(100) NOT NULL COMMENT '위험 구역 이름',
    zone_shape ENUM('rectangle','polygon') NOT NULL DEFAULT 'rectangle' COMMENT 'ROI 형태',
    x_min INT NULL COMMENT '사각형 ROI 시작 X',
    y_min INT NULL COMMENT '사각형 ROI 시작 Y',
    x_max INT NULL COMMENT '사각형 ROI 끝 X',
    y_max INT NULL COMMENT '사각형 ROI 끝 Y',
    polygon_points JSON NULL COMMENT '다각형 ROI 좌표 JSON',
    risk_level TINYINT NOT NULL DEFAULT 3 COMMENT '구역 자체 위험도 1~5',
    description VARCHAR(255) NULL COMMENT '위험 구역 설명',
    is_active BOOLEAN NOT NULL DEFAULT TRUE COMMENT '활성 여부',
    created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
    updated_at DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,

    CONSTRAINT chk_danger_zone_risk_level CHECK (risk_level BETWEEN 1 AND 5),

    CONSTRAINT fk_danger_zones_scenario
        FOREIGN KEY (scenario_id) REFERENCES scenarios(scenario_id)
        ON DELETE CASCADE ON UPDATE CASCADE,

    CONSTRAINT fk_danger_zones_zone
        FOREIGN KEY (zone_id) REFERENCES zones(zone_id)
        ON DELETE SET NULL ON UPDATE CASCADE,

    CONSTRAINT fk_danger_zones_file
        FOREIGN KEY (file_id) REFERENCES upload_files(file_id)
        ON DELETE CASCADE ON UPDATE CASCADE
) COMMENT='위험 구역 ROI 정의';

-- 15. 객체-위험구역 침입 판정 결과 테이블
CREATE TABLE zone_intrusion_results (
    intrusion_result_id INT AUTO_INCREMENT PRIMARY KEY,
    detection_result_id INT NOT NULL COMMENT '어느 객체 탐지 결과인지',
    danger_zone_id INT NOT NULL COMMENT '어느 위험 구역과 비교했는지',
    analysis_run_id INT NOT NULL COMMENT '어느 분석 실행 기준인지',
    file_id INT NOT NULL COMMENT '어느 원본 파일 기준인지',
    frame_id INT NULL COMMENT '영상 프레임 결과일 경우',
    anchor_x INT NOT NULL COMMENT '침입 판단 기준점 X (예: 하단 중앙)',
    anchor_y INT NOT NULL COMMENT '침입 판단 기준점 Y (예: 하단 중앙)',
    is_inside BOOLEAN NOT NULL DEFAULT FALSE COMMENT '위험 구역 내부 여부',
    overlap_ratio DECIMAL(6,4) NULL COMMENT '선택: 박스-ROI 겹침 비율',
    judged_at DATETIME DEFAULT CURRENT_TIMESTAMP COMMENT '판정 시각',

    CONSTRAINT fk_intrusion_detection_result
        FOREIGN KEY (detection_result_id) REFERENCES detection_results(detection_result_id)
        ON DELETE CASCADE ON UPDATE CASCADE,

    CONSTRAINT fk_intrusion_danger_zone
        FOREIGN KEY (danger_zone_id) REFERENCES danger_zones(danger_zone_id)
        ON DELETE CASCADE ON UPDATE CASCADE,

    CONSTRAINT fk_intrusion_analysis_run
        FOREIGN KEY (analysis_run_id) REFERENCES analysis_runs(analysis_run_id)
        ON DELETE CASCADE ON UPDATE CASCADE,

    CONSTRAINT fk_intrusion_file
        FOREIGN KEY (file_id) REFERENCES upload_files(file_id)
        ON DELETE CASCADE ON UPDATE CASCADE,

    CONSTRAINT fk_intrusion_frame
        FOREIGN KEY (frame_id) REFERENCES extracted_frames(frame_id)
        ON DELETE CASCADE ON UPDATE CASCADE
) COMMENT='객체의 위험 구역 진입 판정 결과';

-- 16. 위험 이벤트 로그 테이블
CREATE TABLE risk_events (
    risk_event_id INT AUTO_INCREMENT PRIMARY KEY,
    batch_id INT NOT NULL COMMENT '어느 업로드 배치 기준인지',
    file_id INT NOT NULL COMMENT '어느 원본 파일 기준인지',
    frame_id INT NULL COMMENT '영상 프레임 기준일 경우',
    analysis_run_id INT NOT NULL COMMENT '어느 분석 실행 기준인지',
    detection_result_id INT NOT NULL COMMENT '관련 객체 탐지 결과',
    intrusion_result_id INT NOT NULL COMMENT '관련 침입 판정 결과',
    danger_zone_id INT NOT NULL COMMENT '관련 위험 구역',
    event_type ENUM('zone_intrusion') NOT NULL DEFAULT 'zone_intrusion' COMMENT '이벤트 유형',
    title VARCHAR(100) NOT NULL COMMENT '이벤트 제목',
    message TEXT NULL COMMENT '이벤트 상세 메시지',
    risk_level TINYINT NOT NULL COMMENT '이벤트 위험도 1~5',
    snapshot_path VARCHAR(500) NULL COMMENT '경고 스냅샷/결과 이미지 경로',
    event_status ENUM('new','acknowledged','resolved') NOT NULL DEFAULT 'new' COMMENT '이벤트 상태',
    started_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '이벤트 시작 시각',
    ended_at DATETIME NULL COMMENT '이벤트 종료 시각',
    created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
    updated_at DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,

    CONSTRAINT chk_risk_events_level CHECK (risk_level BETWEEN 1 AND 5),

    CONSTRAINT fk_risk_events_batch
        FOREIGN KEY (batch_id) REFERENCES upload_batches(batch_id)
        ON DELETE CASCADE ON UPDATE CASCADE,

    CONSTRAINT fk_risk_events_file
        FOREIGN KEY (file_id) REFERENCES upload_files(file_id)
        ON DELETE CASCADE ON UPDATE CASCADE,

    CONSTRAINT fk_risk_events_frame
        FOREIGN KEY (frame_id) REFERENCES extracted_frames(frame_id)
        ON DELETE CASCADE ON UPDATE CASCADE,

    CONSTRAINT fk_risk_events_analysis_run
        FOREIGN KEY (analysis_run_id) REFERENCES analysis_runs(analysis_run_id)
        ON DELETE CASCADE ON UPDATE CASCADE,

    CONSTRAINT fk_risk_events_detection_result
        FOREIGN KEY (detection_result_id) REFERENCES detection_results(detection_result_id)
        ON DELETE CASCADE ON UPDATE CASCADE,

    CONSTRAINT fk_risk_events_intrusion_result
        FOREIGN KEY (intrusion_result_id) REFERENCES zone_intrusion_results(intrusion_result_id)
        ON DELETE CASCADE ON UPDATE CASCADE,

    CONSTRAINT fk_risk_events_danger_zone
        FOREIGN KEY (danger_zone_id) REFERENCES danger_zones(danger_zone_id)
        ON DELETE CASCADE ON UPDATE CASCADE
) COMMENT='위험 구역 침입 이벤트 로그';

-- 4단계 인덱스
CREATE INDEX idx_danger_zones_scenario_id ON danger_zones(scenario_id);
CREATE INDEX idx_danger_zones_zone_id ON danger_zones(zone_id);
CREATE INDEX idx_danger_zones_file_id ON danger_zones(file_id);
CREATE INDEX idx_danger_zones_active ON danger_zones(is_active);

CREATE INDEX idx_intrusion_detection_result_id ON zone_intrusion_results(detection_result_id);
CREATE INDEX idx_intrusion_danger_zone_id ON zone_intrusion_results(danger_zone_id);
CREATE INDEX idx_intrusion_analysis_run_id ON zone_intrusion_results(analysis_run_id);
CREATE INDEX idx_intrusion_file_id ON zone_intrusion_results(file_id);
CREATE INDEX idx_intrusion_frame_id ON zone_intrusion_results(frame_id);
CREATE INDEX idx_intrusion_is_inside ON zone_intrusion_results(is_inside);

CREATE INDEX idx_risk_events_batch_id ON risk_events(batch_id);
CREATE INDEX idx_risk_events_file_id ON risk_events(file_id);
CREATE INDEX idx_risk_events_frame_id ON risk_events(frame_id);
CREATE INDEX idx_risk_events_analysis_run_id ON risk_events(analysis_run_id);
CREATE INDEX idx_risk_events_detection_result_id ON risk_events(detection_result_id);
CREATE INDEX idx_risk_events_intrusion_result_id ON risk_events(intrusion_result_id);
CREATE INDEX idx_risk_events_danger_zone_id ON risk_events(danger_zone_id);
CREATE INDEX idx_risk_events_status ON risk_events(event_status);
CREATE INDEX idx_risk_events_started_at ON risk_events(started_at);
